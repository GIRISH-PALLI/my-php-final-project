...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $check_sql = "SELECT * FROM students WHERE (student_id = ? OR email = ?) AND id != ?";
    $stmt = mysqli_prepare($conn, $check_sql);
    mysqli_stmt_bind_param($stmt, "ssi", $student_id, $email, $id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    if (empty($errors)) {
        $sql = "UPDATE students SET student_id = ?, first_name = ?, last_name = ?, email = ?, phone = ?, course = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssi", $student_id, $first_name, $last_name, $email, $phone, $course, $id);
